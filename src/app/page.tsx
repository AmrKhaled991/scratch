...
import { ArrowLeft, ShieldCheck, Truck, HeartPulse, RotateCcw, Tag, Clock } from "lucide-react";
...
const features = [
  { icon: ShieldCheck, title: "جودة صيدلانية", desc: "جميع المنتجات من مصنّعين معتمدين" },
  { icon: Truck, title: "توصيل سريع", desc: "شحن في نفس اليوم للطلبات قبل الساعة 3 مساءً" },
  { icon: HeartPulse, title: "خبراء الصحة", desc: "مراجعة من صيادلة وأخصائيي تجميل" },
  { icon: Clock, title: "دعم 24/7", desc: "فريقنا متاح دائماً لمساعدتك" },
];

const categoryCards = [
  { label: "العناية بالبشرة", value: "skincare", color: "from-rose-50 to-rose-100", textColor: "text-rose-700", img: "https://images.unsplash.com/photo-1620916566398-39f1143ab7be?w=200&h=200&fit=crop" },
  { label: "العناية بالشعر", value: "haircare", color: "from-amber-50 to-amber-100", textColor: "text-amber-700", img: "https://images.unsplash.com/photo-1516975080664-ed2fc6a32937?w=200&h=200&fit=crop" },
  { label: "فيتامينات", value: "vitamins", color: "from-emerald-50 to-emerald-100", textColor: "text-emerald-700", img: "https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=200&h=200&fit=crop" },
  { label: "العناية بالفم", value: "oral-care", color: "from-blue-50 to-blue-100", textColor: "text-blue-700", img: "https://images.unsplash.com/photo-1607613009820-a29f7bb81c04?w=200&h=200&fit=crop" },
];
  ...
  return (
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Cairo', Arial, sans-serif" }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 flex flex-col md:flex-row items-center gap-10">
          <div className="flex-1 text-center md:text-right">
            ...
            <p className="text-emerald-100 text-lg mb-8 max-w-xl md:mr-0 mx-auto">
              اطلب منتجات احترافية للعناية بالبشرة والشعر والفيتامينات مباشرة من المتاجر الصيدلانية الموثوقة. سريع، موثوق، وأصلي دائماً.
            </p>
          ...
          <div className="flex-shrink-0 grid grid-cols-2 gap-3 w-full max-w-xs">
            {categoryCards.slice(0, 4).map((c) => (
              <Link key={c.value} href={`/products?category=${c.value}`} className="rounded-2xl overflow-hidden shadow-lg hover:scale-105 transition-transform">
                <Image src={c.img} alt={c.label} width={150} height={150} className="w-full h-28 object-cover" />
              </Link>
            ))}
          </div>
        </div>
      ...
      <div className="bg-gradient-to-l from-blue-600 to-indigo-600 text-white text-center py-2.5 px-4 text-sm font-medium">
        🎉 قريباً: الدفع الإلكتروني عبر <span className="font-bold underline">PayMob</span> متاح قريباً في التطبيق!
      </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h2 className="text-2xl font-bold text-gray-900 mb-6">تسوّق حسب الفئة</h2>
          ...
          {categoryCards.map((c) => (
            ...
            >
              <div className="w-16 h-16 rounded-2xl overflow-hidden shadow-sm">
                <Image src={c.img} alt={c.label} width={64} height={64} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
              </div>
      ...
      <section className="bg-white border-t border-gray-100 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-10 text-center">سياساتنا</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {policies.map((policy) => (
              <div key={policy.title} className="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-xl bg-emerald-100 flex items-center justify-center">
                    <policy.icon className="w-5 h-5 text-emerald-600" />
                  </div>
                  <h3 className="text-base font-bold text-gray-900">{policy.title}</h3>
                </div>
      ...
      <section className="bg-gradient-to-l from-emerald-600 to-teal-500 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-emerald-100 mb-6 max-w-xl mx-auto">
            تصفح كتالوجنا الكامل من منتجات الصحة والجمال. متاح للطلب بالجملة بأسعار تنافسية للصيدليات.
          </p>
          <Link
            href="/products"
      ...
      <footer className="bg-gray-900 text-gray-400 py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <span className="text-white font-bold text-lg">صيدليات مكه</span>
                <p className="text-sm mt-1">منتجات الصحة والجمال الاحترافية للصيدليات.</p>
