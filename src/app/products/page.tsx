...
function ProductsContent() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === "name") list = [...list].sort((a, b) => a.nameAr.localeCompare(b.nameAr, "ar"));
    return list;
    ...
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Cairo', Arial, sans-serif" }}>
      ...
      <div className="bg-white border-b border-gray-100 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-gray-500 mt-1">تصفح كتالوجنا الكامل من منتجات الصحة والجمال الصيدلانية</p>
        </div>
      </div>
        ...
        <div className="flex gap-6">
          ...
          <aside className="w-48 flex-shrink-0 hidden sm:block">
            <div className="bg-white rounded-2xl border border-gray-100 p-4 sticky top-20">
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">الفئة</h3>
          ...
          <div className="flex-1">
            <p className="text-xs text-gray-400 mb-4">
              يتم عرض <span className="font-semibold text-gray-700">{filtered.length}</span> منتج
            </p>
            {filtered.length === 0 ? (
              <div className="text-center py-20">
                <p className="text-gray-400 text-lg">لم يتم العثور على منتجات.</p>
                <button onClick={() => { setSearch(""); setActiveCategory("all"); }} className="mt-3 text-emerald-600 text-sm hover:underline">
                  مسح الفلاتر
                </button>
              </div>
            ) : (
...
export default function ProductsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-gray-50 flex items-center justify-center text-gray-400">جار التحميل...</div>}>
      <ProductsContent />
    </Suspense>
  );
}
